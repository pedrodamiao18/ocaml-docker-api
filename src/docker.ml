...
(** Debug module with configurable verbosity level.
    Set [verbosity] to control which messages are printed:
...
module Debug = struct
  ...
  let _set_verbosity v = verbosity := v

  let log level place fmt =
    if !verbosity >= level then
      Printf.printf ("[Docker:" ^^ place ^^"] " ^^ fmt ^^ "\n%!")
    else
...
let file_exists path =
  try Unix.access path [Unix.F_OK]; true
  with _ -> false
...
let detect_unix_socket () =
  (* Explicit override *)
  match getenv_opt "DOCKER_HOST" with
  | Some s when String.starts_with ~prefix:"unix://" s ->
  ...
  | _ ->
      ...
      let xdg_runtime_dir =
        match getenv_opt "XDG_RUNTIME_DIR" with
        | Some d -> d
        | None -> "/run/user/" ^ string_of_int uid
      in

      let podman_sock =
        Filename.concat xdg_runtime_dir "podman/podman.sock"
      in

      let docker_sock = "/var/run/docker.sock" in

      if file_exists podman_sock then (
        Debug.log 1 "detect_unix_socket" "Using Podman socket: %s" podman_sock;
        podman_sock
      ) else if file_exists docker_sock then (
        Debug.log 1 "detect_unix_socket" "Using Docker socket: %s" docker_sock;
        docker_sock
      ) else
        failwith "No Podman/Docker socket found"

let default_addr =
  if Sys.win32 then
    ref (
      Unix.ADDR_INET
        (Unix.inet_addr_of_string "127.0.0.1", 2375)
    )
  else
    ref (
      Unix.ADDR_UNIX (detect_unix_socket ())
    )
...
let read_char fn_name fd () =
  let byte = Bytes.create 1 in

  match Unix.read fd byte 0 1 with
  | 0 ->
      Unix.close fd;
      raise (Error (fn_name, "Unexpected EOF"))
  | _ ->
      Bytes.get byte 0


let read_line fn_name fd () =
  let line = Buffer.create 128 in

  let rec loop prev_cr =
    let c = read_char fn_name fd () in

    if prev_cr && c = '\n' then
      ()
    else begin
      if prev_cr then Buffer.add_char line '\r';
      if c <> '\r' then Buffer.add_char line c;
      loop (c = '\r')
    end
  in
...
let read_headers fn_name fd =
  let headers = ref [] in

  (* Status line *)
  let status = read_line fn_name fd () in

  (* Headers *)
  let rec loop () =
    let h = read_line fn_name fd () in
    if h = "" then ()
    else begin
      headers := h :: !headers;
      loop ()
...
let read_exactly fd len =
  ...
  let rec loop off remaining =
    if remaining = 0 then ()
      ...
      Debug.log 2 "read_exactly" "read %d bytes" r;
      if r = 0 then raise End_of_file;
      loop (off + r) (remaining - r)
  in
...
let has_header name value headers =
  List.exists
    (fun h ->
      ...
      | k :: v ->
          ...
          &&
          String.lowercase_ascii (String.trim (String.concat ":" v))
...
let get_header name headers =
  List.find_map
    (fun h ->
      ...
      | [] -> None
      | k :: v ->
          if String.lowercase_ascii (String.trim k) = String.lowercase_ascii name then
            Some (String.trim (String.concat ":" v))
          else None)
    headers

let read_chunked_body fn_name fd =
  let buf = Buffer.create 4096 in

  let rec loop () =
    let chunk_size_hex = read_line fn_name fd () in
    let chunk_size =
      try int_of_string ("0x" ^ chunk_size_hex)
      with Failure _ ->
        Unix.close fd;
        raise (Error ("Docker.read_chunked_body", "Invalid chunk size: " ^ chunk_size_hex))
    in

    if chunk_size > 0 then begin
      Buffer.add_string buf (read_exactly fd chunk_size);
      (* Read the trailing \r\n *)
      ignore (read_exactly fd 2);
      loop ()
    end
  in
...
let read_response fn_name fd =
  let status, h = read_headers fn_name fd in
  Debug.log 1 "read_response" "Status: %d" status;
  Debug.log 2 "read_response" "Headers:\n    %s" (String.concat "\n    " h);
  if status = 204 (* No Content *) || status = 205 (* Reset Content *) || status = 304 (* Not Modified *) then
    status, h, ""
  ...
  else if has_header "Content-Length" "0" h then
    status, h, ""
  else if get_header "Content-Length" h |> Option.map int_of_string <> None then
    status, h, read_exactly fd (int_of_string (Option.get (get_header "Content-Length" h)))
  else if has_header "Transfer-Encoding" "chunked" h then
    status, h, read_chunked_body fn_name fd
  else
    (* No Content-Length and no chunked encoding: read until the end of the stream. *)
    failwith "Docker.read_response: no Content-Length and no chunked encoding, cannot read body without risking to stall"
...
let get fn_name addr url query =
  ...
  Buffer.add_string buf "GET /v1.44";
...
let[@inline] post_header url query =
  let buf = Buffer.create 4096 in
  Buffer.add_string buf "POST /v1.44";
  Buffer.add_string buf url;
  Buffer.add_encoded_query buf query;
  Buffer.add_string buf Docker_config.http11_header;
  buf
...
let post fn_name  addr url query json =
  ...
  (match json with
   ...
   | Some json ->
      ...
      Debug.log 1 "post" "Sending POST request to %s" url;
      let lines = String.split_on_char '\n' (Buffer.contents buf) in
      Debug.log 2 "post" "Headers:\n  %s" (String.concat "\n  " lines);
      Debug.log 2 "post" "Body: %s" (match json with None -> "Empty" | Some j -> Json.to_string j);
      send_buffer fn_name addr buf
...
module Stream = struct
  ...
  (* Read all available RAW data as a list of kind * string pairs. *)
  let read_all_raw st =
    let l = ref [] in
    let rec loop () =
      fill_if_needed st ~timeout:(-1.);
      let available = st.i1 - st.i0 in
      if available > 0 then (
        let s = Bytes.sub st.buf st.i0 available in
        st.i0 <- st.i0 + available;
        l := (Stdout, Bytes.unsafe_to_string s) :: !l;
        loop ()
      );
    in
    loop ();
  ...
  let read_all st =
    if st.tty then read_all_raw st
    else
    let l = ref [] in
...
module Container = struct
  ...
  let attach ?(addr= !default_addr)
    ...
    let buf = Buffer.create 4096 in
    let status, h = read_headers "Docker.Containers.attach" fd in
    ...
    (* Handle Content-Type to determine stream format *)
    let content_type = get_header "Content-Type" h in
    match content_type with
    | None ->
        Unix.close fd;
        raise(Invalid_argument "Docker.Containers.attach: no Content-Type header")
    | Some ct ->
        if ct = "application/vnd.docker.raw-stream" then (
          (* TTY mode: raw stream, no multiplexing headers *)
          Debug.log 2 "Docker.Containers.attach" "Docker.Containers.attach: using raw stream (TTY mode)\n%!";
          Stream.create ~tty:true buf fd
        ) else if ct = "application/vnd.docker.multiplexed-stream" then (
          (* Non-TTY mode: multiplexed stream with 8-byte headers per chunk *)
          Debug.log 2 "Docker.Containers.attach" "Docker.Containers.attach: using multiplexed stream (non-TTY mode)\n%!";
          Stream.create ~tty:false buf fd
        ) else (
          Unix.close fd;
          raise(Invalid_argument
            (Printf.sprintf "Docker.Containers.attach: unsupported Content-Type %S" ct))
        )
  ...
  module Exec = struct
    ...
    let start ?(addr= !default_addr) exec_id =
      ...
      let buf = Buffer.create 4096 in
      let status, _ = read_headers "Docker.Containers.Exec.start" fd in
...
module Image = struct
  ...
  let create ?(addr= !default_addr) ?(platform="linux/amd64") from =
    let q = ["platform", platform] in
    Debug.log 2 "Docker.Image.create" "Creating image with platform: %s" platform;
    Debug.log 1 "Docker.Image.create" "Creating image with source: %s" (match from with
                                                   | Image im -> im.name
                                                   | Src url -> url
                                                   | Stdin _ -> "<stdin>");
    let status = match from with
      ...
      | Src url ->
         ...
         Debug.log 1 "Docker.Image.create" "Creating image from source: %s" url;
         let status, _, _ = response_of_post "Docker.Image.create"
                              addr "/images/create" q None in
         status
      | Stdin img ->
